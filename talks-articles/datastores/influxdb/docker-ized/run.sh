...
if [ -n "${FORCE_HOSTNAME}" ]; then
    if [ "${FORCE_HOSTNAME}" == "auto" ]; then
        #set hostname with IPv4 NIC
        IPv4_NIC_DEVICE=$(ip -o -4 link | grep -v DOWN | grep -v '[0-9]\:\s*lo\:' | head -1 | awk -F': ' '{print $2}')
        HOSTIPNAME=$(ip a show dev $IPv4_NIC_DEVICE | grep inet | grep $IPv4_NIC_DEVICE | sed -e 's/^.*inet.//g' -e 's/\/.*$//g')

        sed -i -r -e "s/^# hostname.*$/hostname = \"${HOSTIPNAME}\"/g" ${CONFIG_FILE}
    else
        sed -i -r -e "s/^# hostname.*$/hostname = \"${FORCE_HOSTNAME}\"/g" ${CONFIG_FILE}
    fi
fi

if [ -n "${REPLI_FACTOR}" ]; then
    sed -i -r -e "s/replication-factor = 1/replication-factor = ${REPLI_FACTOR}/g" ${CONFIG_FILE}
fi
