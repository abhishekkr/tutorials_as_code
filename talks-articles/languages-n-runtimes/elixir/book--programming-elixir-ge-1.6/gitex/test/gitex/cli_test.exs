defmodule GitexCliTest do
  ...
  ## allows capture stdout to test screen printed stuff
  ...
  describe "#parse_args" do
    ...
    property "three vales returned if three passed" do
      check all(
              user <- string(:alphanumeric),
              project <- string(:alphanumeric),
              count <- integer()
            ) do
        assert parse_args([user, project, "#{count}"]) == [
                 user: user,
                 project: project,
                 count: count
               ]
  ...
  describe "#sort_issues" do
    test "sort list of map by reverse created_at" do
      ordered = [
        "2021-04-25T20:24:52Z",
        "2021-04-02T21:12:16Z",
        "2021-03-27T13:17:56Z",
        "2021-03-25T08:50:10Z"
      ]
  ...
  describe "#print_table" do
    test "print formatted issues json" do
      sample_issues = [
        %{
          "number" => 10954,
          "created_at" => "2021-04-25T20:24:52Z",
          "title" => "Allow filtering modules from coverage using regex"
        },
        %{
          "number" => 10865,
          "created_at" => "2021-04-02T21:12:16Z",
          "title" => "Deprecate URI.parse"
        },
        %{
          "number" => 10823,
          "created_at" => "2021-03-27T13:17:56Z",
          "title" => "Wrong \"incompatible types\" warning with tuples"
        },
        %{
          "number" => 10813,
