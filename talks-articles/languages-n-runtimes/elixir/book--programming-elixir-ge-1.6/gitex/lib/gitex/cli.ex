defmodule Gitex.CLI do
  ...
  defp do_parse_args([user, project, count]), do: [user: user, project: project, count: String.to_integer(count)]
  ...
  defp do_print_table([%{"number" => number, "created_at" => created_at, "title" => title} | tail]) do
    IO.puts(" #{String.pad_trailing(to_string(number), 7, " ")} | #{String.pad_trailing(created_at, 23, " ")} | #{title}")
    do_print_table(tail)
  end
  def print_table(lst) do
    IO.puts(" #{String.pad_trailing("#", 7)} | #{String.pad_trailing("created at", 23)} | title")
    IO.puts(" #{String.pad_trailing("", 7, "-")} + #{String.pad_trailing("", 23, "-")} + #{String.pad_trailing("", 50, "-")}")
